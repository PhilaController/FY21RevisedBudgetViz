...
export default {
  ...
  data() {
    return {
      data: require("@/data/historical_spending.json")
    };
  },
  computed: {
    chartOptions() {
      return {
        annotations: {
          ...
          yaxis: [
            {
              y: this.data["spending"][this.data["spending"].length - 3],
              y2: this.data["spending"][this.data["spending"].length - 1],
              borderColor: "#666",
              fillColor: "#666",
              borderWidth: 0,
              opacity: 0.2,
    ...
    series() {
      return [{ name: "", data: this.data["spending"] }];
    }
  }
};
</script>

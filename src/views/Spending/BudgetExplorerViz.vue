...
export default {
  ...
  data() {
    return {
      ...
      gridConfig: {
        "All Changes": { columns: 1, height: 400 },
        "By Department": {
          columns: 4,
          height: 2500,
          groupby: "dept_name"
  ...
  computed: {
    tableColumns() {
      ...
      let cols;
      if (this.viewingMode !== "By Major Class")
        cols = [
          {
            label: "Name",
            field: "name"
    ...
    tableRows() {
      ...
      if (this.viewingMode == "By Major Class")
        grouped = group(this.rawData, d => d.major_class_description);
      else if (this.viewingMode == "By Category")
        grouped = group(this.rawData, d => d.category_code_description);
      else grouped = group(this.rawData, d => d.dept_name);

      let id = 0;
      for (let [k, v] of grouped) {
        row = { id: id, children: [] };
        if (this.viewingMode == "By Major Class")
          row["major_class_description"] = k;
        else row["name"] = k;
        ...
        for (let i = 0; i < v.length; i++) {
          ...
          child["name"] = v[i].name;
          child["major_class_description"] = v[i].major_class_description;
          ...
          if (child["diff"] != 0) row["children"].push(child);
        }
        id += 1;
      }
    ...
    force_type() {
      return this.viewingMode === "By Department" ? "collide" : "charge";
    },
    force_strength() {
      return this.viewingMode === "By Department" ? 0.05 : 0.4;
    }
  },
  watch: {
  ...
  methods: {
    colorPercentDiffCells() {
      if (this.fillColorScale) {
        let rows = $(this.$refs.budgetSpendingTable.$el).find("tr");

        rows.find("th:nth-child(6)").each((i, x) => {
      ...
      */
      return node => {
        ...
        if (layout.size == 1) {
          ...
          if (
            this.viewingMode == "By Major Class" ||
            this.viewingMode == "By Category"
          ) {
            target.y =
              target.y + Math.round(this.yOffsetScale(node.percent_diff));
          }
    ...
    ticked() {
      ...
      */
      ...
      if (
        ...
        let labels = ["Budget increases", "Budget cuts"];
